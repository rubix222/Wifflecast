...
async function maybeSendEventRecap(tournId) {
  ...
  if (!cfg || !cfg.recapTemplateId) { console.warn('maybeSendEventRecap: no recap template configured — will retry next time this event\'s deciding game finishes'); return; }

  const allPids = new Set();
  t.teamIds.forEach(tid => (State.getTeam(tid)?.playerIds || []).forEach(pid => allPids.add(pid)));
  const recipients = resolveRecipientsForPlayerIds(allPids);
  if (!recipients.length) { console.warn('maybeSendEventRecap: no recipients — will retry next time this event\'s deciding game finishes'); return; }
